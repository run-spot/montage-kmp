...
actual fun WantedModalBottomSheet(
    ...
    val bottomSheetState = rememberModalBottomSheetState(
        skipPartiallyExpanded = type.skipPartiallyExpanded(),
        confirmValueChange = type::confirmSheetValueChange
    )

    val configuration = LocalConfiguration.current
    val width = remember(configuration) { configuration.screenWidthDp.dp }

    if (!type.shouldUseSystemBottomSheet()) {
        WantedDraggableModalBottomSheet(
            ...
            dragHandle = if (type.shouldShowDragHandle()) {
            ...
            content = {
                WantedDialogLayout(
                    modifier = Modifier
                        .then(wantedBottomSheetHeightModifier(type))
                        .fillMaxWidth(),
                    modalSize = modalSize,
                    topBar = topBar,
                    content = content,
