...
fun WantedNumberPicker(
    ...
    WantedStringPicker(
        ...
        enableStartIndex = itemList.indexOf(enableMinValue).let { index ->
            if (index == -1) {
                itemList.firstOrNull { it > enableMinValue }?.let(itemList::indexOf) ?: 0
            } else {
                index
            }
        },
        ...
        onSelect = { index, enabled -> onSelect(index, itemList[index], enabled) }
    )
}
    ...
    Text(
        modifier = modifier
            ...
            .graphicsLayer {
                ...
                rotationX = if (rotation > 90 || rotation < -90) 90f else rotation
